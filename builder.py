...
class Builder:
    def __init__(self, target):
        ...
        self.source = os.path.join(target, 'docs_app')
        self.generated = os.path.join(target, 'docs_app/dist')
    ...
    def create_folder(self, id):
        ...
        file = os.path.join(self.output, id + '.html')
    ...
    def build_rxjs(self):
        ...
        dist = os.path.join(self.source, 'dist')
    ...
    def create_sections(self, content):
        ...
        for match in m:
            ...
            replace = '<a name="//apple_ref/cpp/Section/%s" class="dashAnchor"></a>%s' % (quote(title), match[0])
    ...
    def build_guides(self):
        ...
        for guideJson in glob.glob(pattern, recursive=True):
            with open(guideJson, 'rt') as f:
                ...
                content = re.sub(r'href="(api/[a-zA-Z/]*)"', r'href="\1.html"', guide['contents'])
                content = re.sub(r'href="/(api/[a-zA-Z/]*)"', r'href="\1.html"', content)
                content = re.sub(r'href="/(guide/[a-zA-Z/]*)"', r'href="\1.html"', content)
                content = re.sub(r'href="(guide/[a-zA-Z/]*)"', r'href="\1.html"', content)

                content = self.create_sections(content)

                self.write_template(guide['id'], m.groups()[0], content)
    ...
    def build_api_index(self, index):
        ...
        for i in index:
            ...
            content += re.sub(r'href="(api/[a-zA-Z/]*)"', r'href="\1.html"', rendered) + '\n'
        content += '</div></article>'
    ...
    def build_api(self):
        ...
        for apiJson in glob.glob(pattern, recursive=True):
            ...
            with open(apiJson, 'rt') as f:
                ...
                content = re.sub(r'href="(api/[a-zA-Z/]*)', r'href="\1.html', api['contents'])
    ...
    def build_dash_index(self):
        ...
        guides = glob.glob(os.path.join(self.output, '*.html'))
        guides += glob.glob(os.path.join(self.output, 'guide', '**/*.html'), recursive=True)
        for file in guides:
            path = file.replace(self.output + '/', '')
            with open(file, 'rt') as f:
                f = re.findall('<title>(.*)</title>', f.read())

                c.execute("INSERT INTO searchIndex(name, type, path) VALUES ('%s', 'Guide', '%s')" % (f[0].replace('RxJS - ', ''), path))
